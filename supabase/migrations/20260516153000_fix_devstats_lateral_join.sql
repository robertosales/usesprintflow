-- ============================================================
-- HOTFIX #1: corrige devStats em get_sprint_history
--
-- Problema: ARRAY_AGG(h.id) dentro de subquery correlacionada
-- era semanticamente inválido no PostgreSQL — podia retornar
-- realizedHours = 0 para todos os devs ou lançar exceção.
--
-- Solução: substituir por LEFT JOIN LATERAL que agrega as horas
-- por HU antes do GROUP BY por desenvolvedor.
-- ============================================================
...
BEGIN
  ...
  LOOP
    ...
    SELECT jsonb_build_object(
      ...
      -- ── devStats: CORRIGIDO com LEFT JOIN LATERAL ────────────────────────
      -- Antes: ARRAY_AGG(h.id) dentro de subquery correlacionada → inválido.
      -- Agora: lateral_hours agrega SUM(a.hours) por HU antes do GROUP BY,
      --        permitindo SUM correto por desenvolvedor.
      'devStats', (
        ...
        -- LATERAL: agrega horas por HU individualmente antes do GROUP BY
        LEFT JOIN LATERAL (
  ...
  FROM (
    ...
    WHERE s.is_active = FALSE
      AND s.team_id   = ANY(p_team_ids)
      AND (p_team_id IS NULL OR s.team_id = p_team_id)  -- fix: aplica filtro de time no comparativo
      AND (p_cutoff  IS NULL OR s.end_date::DATE >= p_cutoff)
    GROUP BY s.team_id
  ) t_agg;
...
COMMENT ON FUNCTION get_sprint_history IS
  'v2: devStats corrigido com LEFT JOIN LATERAL. Filtro p_team_id aplicado no comparativo.';
