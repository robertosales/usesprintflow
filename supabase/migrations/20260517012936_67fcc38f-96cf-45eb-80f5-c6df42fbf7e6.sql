...
BEGIN
  ...
  FOR v_sprint IN
    ...
    JOIN public.teams t ON t.id = s.team_id AND t.module = 'sala_agil'
  ...
  LOOP
    SELECT jsonb_build_object(
      ...
      'goal', v_sprint.goal,
      'durationDays', GREATEST(COALESCE(EXTRACT(DAY FROM (v_sprint.end_date::timestamptz - v_sprint.start_date::timestamptz))::int, 0), 0),
      ...
      'desvioHoras', COALESCE((
        SELECT ROUND(
          COALESCE(SUM(a.hours), 0)::numeric - COALESCE((
            SELECT SUM(h2.estimated_hours)
            FROM public.user_stories h2
  ...
  sprint_impedimentos AS (
    SELECT fs.id AS sprint_id, COUNT(DISTINCT i.id) AS total_imp
    FROM filtered_sprints fs
    LEFT JOIN public.impediments i
      ON i.sprint_id = fs.id
      OR i.hu_id IN (
        SELECT h.id FROM public.user_stories h WHERE h.sprint_id = fs.id
      )
    GROUP BY fs.id
  )
  SELECT COALESCE(jsonb_agg(
  ...
  FROM (
    SELECT
      fs.team_id,
      COUNT(fs.id) AS total_sprints,
      AVG(COALESCE((
        SELECT SUM(h.story_points)
        FROM public.user_stories h
        WHERE h.sprint_id = fs.id
          AND LOWER(h.status) = ANY(public.status_concluidos())
      ), 0)) AS avg_velocity,
      AVG(CASE WHEN (
          SELECT COUNT(*) FROM public.user_stories h WHERE h.sprint_id = fs.id
        ) = 0 THEN 0
        ELSE 100.0 * (
          SELECT COUNT(*)
          FROM public.user_stories h
    ...
    FROM filtered_sprints fs
    LEFT JOIN sprint_impedimentos si ON si.sprint_id = fs.id
    GROUP BY fs.team_id
  ) t_agg;
...
GRANT EXECUTE ON FUNCTION public.get_sprint_history(UUID[], UUID, DATE) TO service_role;

COMMENT ON FUNCTION public.get_sprint_history(UUID[], UUID, DATE) IS
  'Histórico administrativo de sprints da Sala Ágil, incluindo sprints ativas e sprints já encerradas/vencidas, sem planejamento futuro.';