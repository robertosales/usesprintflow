...
// ---------------------------------------------------------------------------
// STYLE GUIDE — Login (feature/redesign-ui-admin)
...
// UX corporativo:
//   - autocomplete="email" / "current-password" para gestores de senha
//   - type="email" correto para teclados mobile
//   - aria-label nos icones decorativos
//   - Unico CTA principal por tela (sem abas que distraem)
//   - Link de cadastro discreto no rodape (nao compete com o CTA)
// ---------------------------------------------------------------------------
...
const Auth = () => {
  ...
  // Estados compartilhados login / signup
  ...
  const handleLogin = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!email || !password) { toast.error("Preencha todos os campos obrigatórios"); return; }
    setLoading(true);
  ...
  const handleGoogleLogin = async () => {
    ...
    if (result.error) { toast.error("Erro ao fazer login com Google"); setLoading(false); }
  };

  const handleForgotPassword = async () => {
    if (!email) { toast.error("Informe seu e-mail no campo acima para receber o link de redefinição"); return; }
    setLoading(true);
        ...
        <CardContent className="px-8 pb-8 pt-4 space-y-5">
          ...
          {idleTimeout && (
            <div className="flex items-start gap-3 rounded-xl border border-amber-400/40 bg-amber-50 px-4 py-3">
              <Clock className="h-4 w-4 text-amber-500 mt-0.5 shrink-0" aria-hidden="true" />
              <p className="text-sm text-amber-700">
                Sua sessão foi encerrada por <strong>inatividade</strong>. Faça login para continuar.
              </p>
            </div>
          )}
          ...
          {/* Formulário de login — unico CTA principal da tela */}
          <form onSubmit={handleLogin} className="space-y-4" noValidate>
            ...
            {/* Link esqueci senha — secundario, abaixo do CTA */}
