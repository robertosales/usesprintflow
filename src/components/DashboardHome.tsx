...
function KpiCard({
  ...
  progress, progressColor, statusBadge, trend, trendLabel,
}: KpiCardProps) {
  return (
    <div className={cn(
      ...
      <div className="px-5 pt-4 pb-4 flex flex-col gap-2">
        ...
        {/* Progress bar */}
        {progress !== undefined && (
          <Progress value={progress} className={cn("h-1.5 mt-1", progressColor)} />
        )}
      </div>
    </div>
  );
...
function SprintProgressBar({ sprint }: {
  sprint: {
    ...
    isActive: boolean; closedAt?: string | null;
  };
}) {
  ...
  const badgeLabel = isClosed ? "Encerrada"
    : isOverdue ? `+${differenceInDays(today, end)}d atraso`
  ...
  return (
    <div className="bg-card rounded-xl border border-border border-t-[3px] border-t-primary shadow-sm px-5 py-4">
      ...
      {sprint.goal && (
        <p className="text-xs text-muted-foreground mt-2 italic line-clamp-2">"{sprint.goal}"</p>
      )}
    ...
    <div className="flex flex-col gap-5 px-4 sm:px-6 py-6 w-full overflow-x-hidden">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
        <KpiCard
          ...
          value={<span>{doneHUs.length}<span className="text-base font-normal text-muted-foreground">/{sprintHUs.length}</span></span> as any}
