...
export function useUsersAdmin(contractId?: string | null) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      // Se contractId fornecido, busca apenas user_ids desse contrato
      ...
      if (contractId) {
        ...
        allowedUserIds = new Set((ucData ?? []).map((r: any) => r.user_id as string));
      }
      ...
      if (profilesRes.error) { setError(`Erro ao buscar usuários: ${profilesRes.error.message}`); return; }

      const profiles      = (profilesRes.data    || []).filter((p: any) =>
        allowedUserIds ? allowedUserIds.has(p.user_id) : true
      );
      ...
      contractRoles.forEach((cr: any) => { if (cr.user_id) contractRoleMap[cr.user_id] = cr.role; });
      ...
      setUsers(profiles.map((p: any) => {
        ...
        return {
          ...
          team_id: p.team_id || null, team_name: undefined,
  ...
  const saveModuleRoles = async (userId: string, moduleRoles: UserModuleRole[]) => {
    ...
    if (moduleRoles.length > 0) {
      const { error: insErr } = await supabase.from("user_module_roles")
  ...
  const saveContractRole = async (userId: string, contractRole: "admin_contrato" | "member") => {
    const cid = contractId ?? "d59ab6dc-421f-41b4-b415-ae0bc072ebd4";
    const { error } = await supabase.from("user_contracts")
      .upsert({ user_id: userId, contract_id: cid, role: contractRole }, { onConflict: "user_id,contract_id" });
    if (error) throw error;
  };

  const update = async (userId: string, data: {
    display_name?:  string; module_access?: string;
    team_id?:       string | null; is_active?: boolean;
    module_roles?:  UserModuleRole[]; contract_role?: "admin_contrato" | "member";
  }) => {
    try {
      ...
      toast.success("Usuário atualizado"); await load(); return true;
    } catch (e: any) {
      toast.error("Erro ao atualizar usuário: " + (e?.message ?? "")); return false;
    }
  };
  ...
  const resetPassword = async (email: string) => {
    ...
    toast.success(`Link de reset enviado para ${email}`); return true;
  };

  const createUser = async (data: {
    email: string; password: string; display_name: string;
    module_roles: UserModuleRole[]; team_id: string | null;
    ...
    try {
      ...
      await supabase.from("profiles").update({
        display_name: data.display_name, module_access: legacyValue,
        team_id: data.team_id, must_change_password: true,
      }).eq("user_id", authData.user.id);
      await saveModuleRoles(authData.user.id, data.module_roles);
      if (data.contract_role) await saveContractRole(authData.user.id, data.contract_role);
      if (data.team_id) {
        await supabase.from("team_members").upsert({ user_id: authData.user.id, team_id: data.team_id });
      }
      toast.success("Usuário criado com sucesso."); await load(); return true;
    } catch (e: any) {
      toast.error("Erro ao criar usuário: " + (e?.message ?? "")); return false;
    }
  };
