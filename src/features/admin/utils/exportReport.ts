...
export function exportToPDF(payload: ReportPayload) {
  try {
    ...
    toast({ title: "Falha ao gerar PDF", description: "Tente novamente ou contate o suporte.", variant: "destructive" });
  }
}
  ...
  try {
    ...
    if (config.includeSprints && sprints.length > 0) {
      ...
      const rows = sprints.map(s => [
        ...
        s.desvioHoras ?? 0, s.impedimentos,
      ]);
    ...
    toast({ title: "Falha ao gerar Excel", description: "Tente novamente ou contate o suporte.", variant: "destructive" });
  }
}
