...
export const TIPOS_DEMANDA_IMR = [
  ...
  { value: 'apoio_operacional', label: 'Apoio Operacional à Sustentação' },
  { value: 'apuracao_registro_simples', label: 'Apuração especial — registro simples' },
  { value: 'apuracao_relatorio_bd', label: 'Apuração especial — relatório de base de dados' },
  { value: 'apuracao_outras', label: 'Apuração especial — outras' },
  { value: 'assessoria_ux_ui', label: 'Assessoria UX/UI' },
  { value: 'atendimento', label: 'Atendimento' },
  { value: 'atualizacao_arquitetura', label: 'Atualização de arquitetura de deploy de legado' },
  ...
  { value: 'evolutiva_pequeno_porte', label: 'Manutenção evolutiva de pequeno porte' },
  { value: 'evolutiva_pequeno_porte_emergencial', label: 'Manutenção evolutiva de pequeno porte emergencial' },
  { value: 'manutencao_preventiva', label: 'Manutenção preventiva' },
  { value: 'mapeamento_design_thinking', label: 'Mapeamento com Design Thinking' },
  { value: 'modelagem_processos', label: 'Modelagem de processos de negócio' },
  { value: 'planejamento_produto', label: 'Planejamento de Produto' },
  { value: 'suporte_especialista', label: 'Suporte de especialista' },
...
function addBusinessHours(from: Date, hours: number): Date {
  ...
  const WORK_START = 8, WORK_END = 18, HOURS_PER_DAY = 10;
  const result = new Date(from);
  let remaining = hours;
...
export const EVENTOS_CONFIG: EventoConfig[] = [
  { codigo: 'E1', descricao: 'Profissional inadequado/sem qualificação por reincidência', redutor: 0.1, unidade: 'por dia útil', incidencia: 'integral' },
  { codigo: 'E2', descricao: 'Vazamento de informações confidenciais', redutor: 3.0, unidade: 'por ocorrência', incidencia: 'integral' },
  { codigo: 'E3', descricao: 'Falha grosseira detectada apenas em produção', redutor: 0.5, unidade: 'por ocorrência', incidencia: 'integral' },
  { codigo: 'E4', descricao: 'Hard code de parâmetros importantes', redutor: 0.1, unidade: 'por ocorrência', incidencia: 'integral' },
  { codigo: 'E5', descricao: 'Artefatos não atualizados após manutenção', redutor: 0.1, unidade: 'por ocorrência', incidencia: 'limitada' },
  { codigo: 'E6', descricao: 'Demanda suspensa sem justificativa ou sem previsão', redutor: 0.1, unidade: 'por ocorrência', incidencia: 'limitada' },
  { codigo: 'E7', descricao: 'Reincidência de defeito impeditivo', redutor: 0.2, unidade: 'por ocorrência', incidencia: 'limitada' },
  { codigo: 'E8', descricao: 'SLA em atraso há mais de 60 dias corridos', redutor: 0.2, unidade: 'por ocorrência', incidencia: 'limitada' },
  { codigo: 'E9', descricao: 'Composição mínima de equipe não mantida', redutor: 0.1, unidade: 'por dia útil após notificação', incidencia: 'limitada' },
  { codigo: 'E10', descricao: 'Ausência de profissional causando prejuízo em OS', redutor: 0.05, unidade: 'por dia útil', incidencia: 'limitada' },
  { codigo: 'E11', descricao: 'Compartilhamento indevido de profissional entre equipes', redutor: 0.05, unidade: 'por dia útil', incidencia: 'limitada' },
  { codigo: 'E13', descricao: 'Atraso na alocação de equipe após abertura de OS', redutor: 0.2, unidade: 'por dia de atraso', incidencia: 'limitada' },
  { codigo: 'E14', descricao: 'Reincidência de defeito impeditivo de sprint anterior', redutor: 0.2, unidade: 'por ocorrência', incidencia: 'limitada' },
];
...
export function getSLAStatusDemanda(createdAt: string, prazoSolucao: string | null, situacao: string) {
  ...
  if (diasAtraso > 60) return { status: 'glosa_e8' as const, cor: 'red' as const, label: `Glosa E8 — ${diasAtraso} dias de atraso`, percentConsumed, diasAtraso };
  if (diasAtraso >= 45) return { status: 'alerta_e8' as const, cor: 'orange' as const, label: `Alerta preventivo — ${diasAtraso} dias de atraso`, percentConsumed, diasAtraso };
  if (now > prazo) return { status: 'excedido' as const, cor: 'red' as const, label: 'Prazo excedido', percentConsumed, diasAtraso };
  if (percentConsumed >= 75) return { status: 'risco' as const, cor: 'yellow' as const, label: `75% do prazo consumido`, percentConsumed };
