...
export function DemandaForm({ open, onClose, onSubmit }: Props) {
  ...
  const handle = async () => {
    if (!form.rhm.trim()) { toast.error("Preencha os campos obrigatórios"); return; }
    setLoading(true);
    await onSubmit(form);
    ...
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent>
        ...
        <div className="space-y-3">
          <div><Label>RHM *</Label><Input value={form.rhm} onChange={e => setForm(p => ({ ...p, rhm: e.target.value }))} placeholder="RHM-001" /></div>
          <div><Label>Projeto</Label><Input value={form.projeto} onChange={e => setForm(p => ({ ...p, projeto: e.target.value }))} placeholder="Nome do projeto" /></div>
